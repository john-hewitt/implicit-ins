...
accelerate launch \
    ...
    --weight_decay 0. \
    --num_train_epochs 5 \
    --output_dir output/olmo_lima_${MODELNAME}_${MODEL_SIZE}_try7/ \
    --with_tracking \
...
model=olmo_lima_${MODELNAME}_${MODEL_SIZE}_try7; python -m eval.val_eval.run_eval --model_name_or_path output/${model}/  --tokenizer_name_or_path output/${model}/ --save_dir results/val_eval/${model}/      --eval_batch_size 10          --use_chat_format     --chat_formatting_function eval.templates.create_prompt_with_tulu_chat_format --use_vllm
